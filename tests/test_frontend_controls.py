...
class FrontendControlTests(unittest.TestCase):
    ...
    def test_draw_count_range_is_wired(self):
        html = (ROOT / 'static/index.html').read_text(encoding='utf-8')
        script = (ROOT / 'static/app.js').read_text(encoding='utf-8')
        self.assertIn('id="count"', html)
        self.assertIn('id="minus"', html)
        self.assertIn('id="plus"', html)
        self.assertIn('id="roll-range-toggle"', html)
        self.assertIn('id="roll-range-fields"', html)
        self.assertIn('class="roll-range-fields" hidden', html)
        self.assertIn('id="roll-min"', html)
        self.assertIn('id="roll-max"', html)
        self.assertIn('id="roll-min" type="number"', html)
        self.assertIn('id="roll-max" type="number"', html)
        self.assertIn('id="roll-min" type="number" min="1" max="500000"', html)
        self.assertIn('id="roll-max" type="number" min="1" max="500000"', html)
        self.assertIn('value="1"', html)
        self.assertNotIn('id="count-range"', html)
        self.assertIn('function parseCountRange', script)
        self.assertNotIn('function randomCountInRange', script)
        self.assertIn('const count = countValue()', script)
        self.assertIn('roll_range: [range.min, range.max]', script)
        self.assertIn('batch.roll_counts', script)
        self.assertIn('function toggleRollRange', script)
        self.assertIn('function updateRollRangeSummary', script)
        self.assertIn("$('roll-range-toggle').addEventListener", script)
        self.assertIn('const MAX_ROLL_COUNT = 500000', script)
        self.assertIn('const DRAW_TIMEOUT_MS = 300000', script)
        self.assertIn("path === 'api/draw' || path === 'api/reroll'", script)
        self.assertNotIn('batch.accelerated', script)
        self.assertNotIn('等效加速', script)
        self.assertIn('摇动次数范围无效', script)
